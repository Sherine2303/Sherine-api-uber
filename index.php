        ...
        case "chauffeurs":
            switch($method) {
                ...
                case "PUT":
                    if (isset($url[1])) {
                    $data = json_decode(file_get_contents("php://input"),true);
                    $chauffeursController->updateChauffeur($url[1],$data);
                ...
                case "DELETE":
                    if (isset($url[1])) {
                        $chauffeursController->deleteChauffeur($url[1]);
                }else {
                    http_response_code(400);
                    echo json_encode(["message" => "ID du chauffeur manquant dans l'URL"]);
                }
        ...
        case "clients":
            switch($method){
                ...
                case "PUT":
                    if (isset($url[1])) {
                    $data = json_decode(file_get_contents("php://input"),true);
                    $clientsController->updateClient($url[1],$data);
                    echo json_encode($data);
                }else {
                    http_response_code(400);
                ...
                case "DELETE":
                if (isset($url[1])) {
                    $clientsController->deleteClient($url[1]);
                } else {
                    http_response_code(400);
                    echo json_encode(["message" => "ID du client manquant dans l'URL"]);
                }
